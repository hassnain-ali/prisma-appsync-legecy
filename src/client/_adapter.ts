...
import { PrismaAppSyncOperations, AuthModes, Operations } from "./_constants";
...
export class PrismaAppSyncAdapter {
    ...
    constructor(event: any, options: AdapterOptions) {
        this.operation = null as any;
        this.model = String();
        this.requestSetPaths = [];
        this.args = {};
        this.authIdentityType = null as any;
    ...
    private detectCallerIdentity(event: any) {
        if (
            typeof event.identity === "undefined" ||
            !event.identity ||
            event.identity.length < 1
        ) {
            ...
            this.authIdentityObj =
                !event.request || !event.request.headers
                    ? {}
                    : {
                          ...(typeof event.request.headers["x-api-key"] !==
                              "undefined" && {
                              requestApiKey: event.request.headers["x-api-key"],
                          }),
                          ...(typeof event.request.headers["user-agent"] !==
                              "undefined" && {
                              requestUserAgent:
            ...
            typeof event.identity["cognitoIdentityAuthProvider"] !==
                "undefined" ||
            ...
            throw new InternalError(
                `Couldn't detect caller identity from: ${JSON.stringify(event.identity)}`,
            );
        }
    }
    ...
    private verifyIntegrity(event: any) {
        if (!event.info || !event.info.fieldName) {
            throw new InternalError(
                `Incomplete event input received (missing info): ${JSON.stringify(event)}`,
            );
        }

        if (!event.arguments) {
            throw new InternalError(
                `Incomplete event input received (missing arguments): ${JSON.stringify(event)}`,
            );
        }

        if (!event.info.selectionSetList) {
            throw new InternalError(
                `Incomplete event input received (missing selectionSetList): ${JSON.stringify(event)}`,
            );
        }
    }
    ...
    private parseRequest(event: any) {
        ...
        const isCustomResolver: boolean =
            typeof this.customResolvers[fieldName] !== "undefined";
        ...
        if (isCustomResolver) {
            ...
            const operation: any = PrismaAppSyncOperations.find(
                (op: string) => {
            ...
            if (
                !this.config ||
                typeof this.config.prismaClientModels === "undefined" ||
    ...
    private parseArgs(event: any) {
        this.args = merge(
            {},
            this.args,
            this.parseSelectionList(event.info.selectionSetList),
        );
        ...
        if (this.operation !== Operations.custom) {
            ...
            if (
                typeof event.arguments.orderBy !== "undefined" &&
                _getOrList.includes(this.operation)
            ) {
                this.args.orderBy = this.parseOrderBy(event.arguments.orderBy);
            }

            if (
                this.operation === Operations.list ||
    ...
    private getInclude(parts: any): any {
        const field = parts[0];
        const value = parts.length > 1 ? this.getSelect(parts.splice(1)) : true;
    ...
    private parseSelectionList(selectionSetList: any) {
        ...
        for (let i = 0; i < selectionSetList.length; i++) {
            ...
            if (!parts.includes("__typename")) {
                if (parts.length > 1)
                    args = merge({}, args, this.getInclude(parts));
        ...
        if (args.include) {
            for (const include in args.include) {
                if (typeof args.select[include] !== "undefined")
                    delete args.select[include];
            }
