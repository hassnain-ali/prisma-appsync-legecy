...
export class PrismaAppSyncCompiler {
    ...
    constructor(dmmf: DMMF.Document, options: CompilerOptions) {
        this.options = {
            ...
            outputDir:
                options.outputDir ||
                join(process.cwd(), "generated/prisma-appsync"),
        ...
        this.data = {
            models: [],
            enums: [],
            directiveAliases: merge(
                { default: String() },
                this.options.directiveAliases,
            ),
            customResolvers: [],
        };
    ...
    public async makeSchema(customSchemaPath?: string): Promise<this> {
        ...
        const generatedSchema = await readFile(generatorSchemaPath, {
            encoding: "utf-8",
        });
        ...
        const prettySchema = await prettier.format(mergedSchema, {
            semi: false,
            parser: "graphql",
            tabWidth: 4,
            trailingComma: "none",
    ...
    public getClientConfig(): string {
        ...
        for (let i = 0; i < this.data.models.length; i++) {
            ...
            if (model.name !== model.pluralizedName) {
                config["prismaClientModels"][model.pluralizedName] =
                    model.prismaRef;
            }
    ...
    public async makeDocs(): Promise<this> {
        ...
        await this.makeFile(join(__dirname, "./templates/docs/README.md.njk"), {
            outputDir: "docs",
        });
    ...
    private parseDMMF(): this {
        ...
        this.dmmf.datamodel.models.forEach((model: DMMF.Model) => {
            const modelDirectives: DMMFPAS_Directives =
                this.getModelDirectives(model);
            ...
            if (!this.isIgnored(model)) {
                model.fields.forEach((field: DMMF.Field) => {
                    const fieldDirectives: DMMFPAS_Directives =
                        this.getFieldDirectives(field, model);
                    ...
                    if (!field.isGenerated && !this.isIgnored(field)) {
                        fields.push({
                            ...
                            ...(field.relationName && {
                                relation: {
                                    name: this.getFieldRelationName(
                                        field,
                                        model,
                                    ),
                ...
                this.data.models.push({
                    ...
                    fields: fields,
                    subscriptionFields: this.filterSubscriptionFields(fields, [
    ...
    private filterSubscriptionFields(
        ...
        while (shouldContinue) {
            ...
            if (hasRemainingFields && isBelowMaxFieldsLimit) {
                ...
                const destroyIndex = subFields.findIndex(
                    (s) => s.name === "uuid",
                );
    ...
    private async makeFile(
        ...
        const inputContent: string = await readFile(inputFile, {
            encoding: "utf8",
        });
        ...
        const outputFilePath = join(
            ...
            outputFileOptions && outputFileOptions.outputDir
                ? outputFileOptions.outputDir
                : ``,
            outputFilename,
        );
    ...
    private getFieldRelationKind(field: DMMF.Field): "one" | "many" {
        return field.relationFromFields && field.relationFromFields.length === 1
            ? "one"
            : "many";
    }
    ...
    private getModelDirectives(model: DMMF.Model): DMMFPAS_Directives {
        return this.getDirectives(
            model,
            this.options.directivesPriorityScheme["model"],
        );
    }
    ...
    private isIgnored(node: DMMF.Field | DMMF.Model): boolean {
        ...
        if (
            typeof node["documentation"] !== "undefined" &&
    ...
    private getFieldDirectives(
        ...
        if (field.relationName) {
            const relationModel: DMMF.Model | false = this.getModelIfExists(
                field.type,
            );

            if (relationModel) {
                const modelDirectives = this.getModelDirectives(model);
                const relationModelDirectives =
                    this.getModelDirectives(relationModel);
                const fieldDirectives: DMMFPAS_Directives = {
                    ...(modelDirectives["type"] !==
                        relationModelDirectives["type"] && {
                        field: relationModelDirectives["type"],
                    }),
                    ...(modelDirectives["mutation"] !==
                        relationModelDirectives["mutation"] && {
                        mutation: relationModelDirectives["mutation"],
                    }),
                    ...(modelDirectives["create"] !==
                        relationModelDirectives["create"] && {
                        create: relationModelDirectives["create"],
                    }),
                    ...(modelDirectives["update"] !==
                        relationModelDirectives["update"] && {
        ...
        return this.getDirectives(
            field,
            this.options.directivesPriorityScheme["field"],
        );
    }

    // Return DMMF model from model name
    private getModelIfExists(name: string): DMMF.Model | false {
        const modelIndex = this.dmmf.datamodel.models.findIndex(
            (model: DMMF.Model) => {
    ...
    private getDirectives(
        ...
        Object.keys(this.data.directiveAliases).forEach((alias) => {
            if (alias === "default")
                ...
                directiveAliases[`@@${alias}`] =
                    this.data.directiveAliases[alias];
        });
