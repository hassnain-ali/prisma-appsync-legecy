    ...
    async onGenerate(options: any) {
        if (options.generator.output) {
            try {
                ...
                if (debug) {
                    console.log(
                        `[Prisma-AppSync] Generator config: `,
                ...
                Object.keys(options.generator.config)
                    ...
                    .forEach((cfg: string) => {
                        ...
                        const splitIndex: number =
                            cfg.indexOf(aliasPrefix) + aliasPrefix.length;
                ...
                // Generate schema
                await compiler.makeSchema(
                    options.generator.config.customSchema
                );
                ...
                // Generate resolvers
                await compiler.makeResolvers(
                    options.generator.config.customResolvers
                );
                ...
                console.error(
                    "Error: unable to compile files for Prisma AppSync Generator"
                );
                throw e;
            }
        } else {
            throw new Error(
                "No output was specified for Prisma AppSync Generator"
            );
        }
    },
});
